...
* **Microsoft Fabric** – analyze data and generate insights using Fabric data agents
...
* **Microsoft Fabric integration** – analyze data and generate insights using Fabric data agents
...
![Snippy Run](https://raw.githubusercontent.com/Azure-Samples/snippy/gk/fabric/.github/assets/deep-wiki-run.png)

Experience Snippy's capabilities in generating AI-assisted documentation, semantic searches, and deep insights powered by Microsoft Fabric integration.

### Microsoft Fabric Integration

Snippy integrates with Microsoft Fabric to enable powerful data analysis capabilities. To set up Fabric integration:

1. **Create a Fabric Data Agent**:
   - Go to [Microsoft Fabric](https://app.fabric.microsoft.com)
   - Create a new Data Agent in your workspace
   - Configure the agent with your data sources and access permissions
   - Publish the agent to get the endpoint URL

2. **Get Connection Details**:
   - From your published Fabric data agent endpoint URL:
     ```
     https://<environment>.fabric.microsoft.com/groups/<workspace_id>/aiskills/<artifact-id>
     ```
   - Note down the `workspace_id` and `artifact-id`

3. **Create Connection in Azure AI Foundry**:
   - Go to your Azure AI Foundry project
   - Navigate to "Connected Resources" → "Add"
   - Select "Microsoft Fabric"
   - Add a new connection with:
     - Name: `fabric-deep-research` (or your preferred name)
     - Workspace ID: Your Fabric workspace ID
     - Artifact ID: Your Fabric artifact ID
   - Mark both values as secrets

4. **Update Configuration**:
   - Add the Fabric connection name to your `local.settings.json`:
     ```json
     {
       "Values": {
         "FABRIC_CONNECTION_NAME": "fabric-deep-research"
       }
     }
     ```

5. **Use in Snippy**:
   - The deep wiki agent will automatically use Fabric for data analysis
   - Ask questions about your data through the agent
   - Get insights and visualizations based on your Fabric data

> **Note**: Ensure your Fabric data agent and Azure AI Agent are in the same tenant, and users have appropriate RBAC roles (`AI Developer` minimum) and data access permissions.
